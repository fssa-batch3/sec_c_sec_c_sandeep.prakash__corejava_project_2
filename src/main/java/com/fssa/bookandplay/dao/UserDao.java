	...
	public static boolean updateUser(User user) throws DAOException, SQLException {
		...
		try (Connection con = ConnectionUtil.getConnection()) {
			...
			try (CallableStatement callableStatement = con.prepareCall(storedProcedureCall)) {
				...
				callableStatement.setString(6, hashPassword(user.getPassword()));
				callableStatement.setString(7, user.getImage());
				callableStatement.setBoolean(8, user.getPlayerStatus());
				callableStatement.setString(9, user.getDisplayName());
				callableStatement.setLong(10, user.getAge());
				callableStatement.setString(11, user.getGender());
				callableStatement.setString(12, user.getLocation());
				callableStatement.setTime(13, startTimeTs1);
				callableStatement.setTime(14, endTimeTs1);
				callableStatement.setString(15, user.getAbout());
				if(user.getKnownSports()!=null) {
					String sportsKnownStr = String.join(",", user.getKnownSports());
					callableStatement.setString(16, sportsKnownStr);
					}
					else 
					{
